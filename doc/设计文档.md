...
| 3 | `generate` | 双 agent + 工具 | `02_model.json` | `03_config.json` | 任务流生成 Agent 生成状态机/流转；规则生成 Agent 生成各类规则配置 |
| 4 | `review` | 规则审查 agent + 确定性工具 | `03_config.json` | `04_review.json` | 规则审查 Agent 调用确定性审查工具，汇总错误、风险、建议和人工确认点 |
...
└── priority_rules: list[Rule]         # 优先级规则
```
...
├── summary: str                  # 规则审查 Agent 生成的自然语言总结
└── passed: bool
```
...
- 凭证：从环境变量 `ANTHROPIC_API_KEY` 读取；自然语言场景解析缺凭证时直接 fail 并给出明确提示；`generate/review` 保留确定性 fallback，保证 `--from-json` 路径可跑通。
- agent 定义于 `agents/`，包含：独立 system prompt、输出 JSON schema 约束、允许调用的 tools 白名单。
- agent 输出统一要求为符合对应 pydantic schema 的 JSON，由确定性代码二次校验，校验失败即该环节 failed。

| Agent | 职责 | 允许的 tools |
|-------|------|--------------|
| `ScenarioParsingAgent` | 读取结构化/半结构化业务输入；识别场景对象、任务步骤、业务规则和异常分支；输出标准场景模型 `ScenarioModel` | （仅文本理解，无外部 tool） |
| `TaskFlowGenerationAgent` | 根据场景模型生成任务状态机；建立状态节点和流转关系；标记正常流、异常流和取消流 | `check_reachability` |
| `RuleGenerationAgent` | 根据场景模型和状态流转生成规则配置；将业务约束转成结构化规则；输出机器人、工作站、电量、异常、取消、优先级等规则 | `validate_rule_config` / `check_rule_conflict` |
| `RuleReviewAgent` | 调用确定性审查工具；汇总审查结果；给出错误、风险和建议；对需要人工确认的规则给出说明 | 确定性 reachability / conflict / schema 工具 |

确定性环节（model、sandbox）不依赖 agent；`review` 的底层检查仍是确定性工具，由 `RuleReviewAgent` 统一组织报告。
...
│   │   ├── ingest_agent.py       # ScenarioParsingAgent
│   │   ├── task_flow_agent.py    # TaskFlowGenerationAgent
│   │   ├── rule_agent.py         # RuleGenerationAgent
│   │   └── review_agent.py       # RuleReviewAgent
│   ├── stages/
│   │   ├── __init__.py
